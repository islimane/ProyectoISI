...
var Game_ = new function() {                                                                  
  ...
  this.initialize = function(canvasElementId,sprite_data,callback) {
	...
	$(window).keydown(function(event){
		...
		Game_.keys[KEY_CODES[event.which]] = true;
		return false;
	    }
	});
	...
	$(window).keyup(function(event){
          if (focusCanvas)
	    if (KEY_CODES[event.which]) {
		Game_.keys[KEY_CODES[event.which]] = false;
		return false;
	    }
	});
...
var TitleScreen = function TitleScreen(title,subtitle,callback) {
	...
	ctx.fillText(title,Game_.width/2,Game_.height/2);

	ctx.font = "bold 20px bangers";
	ctx.fillText(subtitle,Game_.width/2,Game_.height/2 + 40);
    };
};
...
var TouchControls = function() {
	...
	var yLoc = Game_.height - unitWidth;
	this.drawSquare(ctx,gutterWidth,yLoc,"\u25C0", Game_.keys['left']);
	this.drawSquare(ctx,unitWidth + gutterWidth,yLoc,"\u25B6", Game_.keys['right']);
	this.drawSquare(ctx,4*unitWidth,yLoc,"A",Game_.keys['fire']);
	...
	Game_.keys['left'] = false;
	Game_.keys['right'] = false;
	for(var i=0;i<e.targetTouches.length;i++) {
	    ...
	    x = touch.pageX / Game_.canvasMultiplier - Game_.canvas.offsetLeft;
	    if(x < unitWidth) {
		Game_.keys['left'] = true;
	    } 
	    if(x > unitWidth && x < 2*unitWidth) {
		Game_.keys['right'] = true;
	    } 
	}
	...
	if(e.type == 'touchstart' || e.type == 'touchend') {
		...
		x = touch.pageX / Game_.canvasMultiplier - Game_.canvas.offsetLeft;
		if(x > 4 * unitWidth) {
		    Game_.keys['fire'] = (e.type == 'touchstart');
		}
	    }
	}
    };
    ...
    Game_.canvas.addEventListener('touchstart',this.trackTouch,true);
    Game_.canvas.addEventListener('touchmove',this.trackTouch,true);
    Game_.canvas.addEventListener('touchend',this.trackTouch,true);
    ...
    // offset.
    Game_.playerOffset = unitWidth + 20;
