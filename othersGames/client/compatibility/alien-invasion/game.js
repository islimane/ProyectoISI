...
var startGame_ = function() {
    ...
    Game_.setBoard(3,new TitleScreen("Alien Invasion", 
...
var playGame = function() {
    ...
    Game_.setBoard(3,board);
    Game_.setBoard(5,new GamePoints(0));
};
...
var winGame = function() {
    Meteor.call("matchFinish", Session.get("current_game"), Game_.points);
    Game_.setBoard(3,new TitleScreen("You win!", 
...
var loseGame = function() {
    Meteor.call("matchFinish", Session.get("current_game"), Game_.points);
    Game_.setBoard(3,new TitleScreen("You lose!", 
                                    "Press fire to play again",
                                    playGame));
};
...
var Starfield = function(speed,opacity,numStars,clear) {
    ...
    var stars = $('<canvas/>')
	.attr('width', Game_.width)
	.attr('height', Game_.height)[0];
...
var PlayerShip = function() { 
    ...
    this.x = Game_.width/2 - this.w / 2;
    this.y = Game_.height - Game_.playerOffset - this.h;

    this.step = function(dt) {
	if(Game_.keys['left']) { this.vx = -this.maxVel; }
	else if(Game_.keys['right']) { this.vx = this.maxVel; }
	else { this.vx = 0; }

	this.x += this.vx * dt;

	if(this.x < 0) { this.x = 0; }
	else if(this.x > Game_.width - this.w) { 
	    this.x = Game_.width - this.w;
	}

	this.reload-=dt;
	if(Game_.keys['fire'] && this.reload < 0) {
	    // Esta pulsada la tecla de disparo y ya ha pasado el tiempo reload
	    Game_.keys['fire'] = false;
	    this.reload = this.reloadTime;
...
/*$(function() {
    Game_.initialize("game_",sprites,startGame);
});*/
